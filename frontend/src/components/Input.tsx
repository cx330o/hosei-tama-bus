...
const Input = () => {
  ...
  const handleFilesSelect = (event) => { setFiles((prev) => [...prev, ...Array.from(event.target.files)]); };
  const handleRemoveFile = (fileToRemove) => { setFiles(files.filter((file) => file !== fileToRemove)); };
  const handlePaste = (event) => {
    const items = event.clipboardData.items;
    for (const item of items) {
      if (item.type.startsWith("image/")) { const file = item.getAsFile(); setFiles((prev) => [...prev, file]); event.preventDefault(); }
    }
  };
