...
const useWebSocket = (onMessageReceived) => {
  ...
  useEffect(() => {
    function connect() {
      ...
      wsRef.current = ws;
      ws.onopen = () => { console.log("WebSocket connected"); reconnectDelay.current = INITIAL_RECONNECT_DELAY; };
      ws.onmessage = (event) => {
        const data = JSON.parse(event.data);
        if (data.type === "newMessage") onMessageReceived(data.message);
      };
      ws.onclose = () => { console.log(`WebSocket disconnected. Reconnecting in ${reconnectDelay.current}ms...`); scheduleReconnect(); };
