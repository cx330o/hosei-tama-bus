...
// Group flat JOIN rows into nested message objects with their files
...
function getMessages({ cursor, limit = 20 } = {}) {
  ...
  // Fetch message IDs with cursor-based pagination
  let idSql = `SELECT DISTINCT m.id FROM Message m
    WHERE m.expiration_time > ?`;
  const idParams = [currentTime];

  if (cursor) {
    idSql += ` AND m.creation_time < ?`;
    idParams.push(cursor);
  }
